...
let newUniversityArray = graduates.map(name => {
  return universities.push(name.university);
});
...
let firstNameEmail = graduates.map(name => {
...
let containsUni = graduates.filter(name => {
  if (name.university.startsWith("Uni")) {
    return uni.push(name.university);
  }
});
...
let bothNames = zooAnimals.map(name => {
...
let namesLowerCase = zooAnimals.map(name => {
...
let lowerCount = zooAnimals.filter(name => {
  if (name.population < 5) {
    return lowerPopulation.push(name);
  }
});
...
const populationTotal = zooAnimals.reduce((total, name) => {
